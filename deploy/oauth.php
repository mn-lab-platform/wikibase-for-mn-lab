...
class MW_OAuth {
	...
	function setTokenCookies( $key , $secret , $expires ) {
		$opts = [
			'expires' => $expires ,
		...
		setcookie( 'tokenKey' , $key , $opts ) ;
		setcookie( 'tokenSecret' , $secret , $opts ) ;
	}
	function applyCurlDefaults( $ch , $extra_headers = [] ) {
	...
	function resolveInternalUrl( $url ) {
		...
		$api_host = parse_url( $this->apiUrl ?? '' , PHP_URL_HOST ) ;
		$oauth_host = parse_url( $this->mwOAuthUrl ?? '' , PHP_URL_HOST ) ;
		$our_hosts = array_filter( [ $api_host , $oauth_host ] ) ;
		if ( !$host || !in_array( $host , $our_hosts , true ) ) {
			return [ $url , [] ] ;
		}
		$internal = getenv( 'WIKIBASE_INTERNAL_URL' ) ;
		if ( $internal === false || $internal === '' ) {
			$internal = 'http://wikibase' ;
		}
		$internal = rtrim( $internal , '/' ) ;
		$rewritten = preg_replace( '#https?://' . preg_quote( $host , '#' ) . '#', $internal , $url , 1 ) ;
		return [
			$rewritten ,
			[
				'Host: ' . $host ,
	...
	function fetchAccessToken() {
		...
		$ch = curl_init();
		list( $curl_url , $extra_headers ) = $this->resolveInternalUrl( $url ) ;
		curl_setopt( $ch, CURLOPT_URL, $curl_url );
		$this->applyCurlDefaults( $ch , $extra_headers ) ;
		$data = curl_exec( $ch );
		if ( !$data ) {
			header( "HTTP/1.1 500 Internal Server Error" );
			throw new Exception( 'Curl error fetching access token: ' . htmlspecialchars( curl_error( $ch ) ) );
		}
		curl_close( $ch );
		$token = json_decode( $data );
		if ( $token === null ) {
			parse_str( $data, $parsed_data );
			if ( isset( $parsed_data['oauth_token'] ) && isset( $parsed_data['oauth_token_secret'] ) ) {
				$token = new stdClass();
				$token->key = $parsed_data['oauth_token'];
				$token->secret = $parsed_data['oauth_token_secret'];
			}
		}
		if ( is_object( $token ) && isset( $token->error ) ) {
			header( "HTTP/1.1 500 Internal Server Error" );
			throw new Exception( 'Error retrieving token2: ' . htmlspecialchars( is_string( $token->error ) ? $token->error : json_encode( $token ) ) );
		}
		if ( !is_object( $token ) || !isset( $token->key ) || !isset( $token->secret ) ) {
			header( "HTTP/1.1 500 Internal Server Error" );
			throw new Exception( 'Invalid response from token request: ' . htmlspecialchars( $data ) );
		}
		...
		if ( $this->use_cookies ) {
			...
			$this->setTokenCookies( $_SESSION['tokenKey'] , $_SESSION['tokenSecret'] , $t ) ;
		}
		session_write_close();
	}
	/**
...
//		global $gConsumerSecret, $gTokenSecret;
	...
	function doAuthorizationRedirect($callback='') {
		...
		$token = json_decode( $data );
		if ( $token === NULL ) {
			$debug_url = curl_getinfo($ch, CURLINFO_EFFECTIVE_URL);
			throw new Exception("CRASH_URL: " . $debug_url . " | RESPONSE: " . $data);
	...
	function doIdentify() {
		...
		list( $curl_url , $extra_headers ) = $this->resolveInternalUrl( $url ) ;
		curl_setopt( $ch, CURLOPT_URL, $curl_url );
		$this->applyCurlDefaults( $ch , array_merge( [ $header ] , $extra_headers ) ) ;
		$data = curl_exec( $ch );
		if ( !$data ) {
			header( "HTTP/1.1 500 Internal Server Error" );
			throw new Exception ( 'Curl error: ' . htmlspecialchars( curl_error( $ch ) ) ) ;
